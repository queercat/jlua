...
export const lex = (source: string, offset: number = 0, matches: Token[] = []): Token[] => {
    ...
    if (!symbol) return [...matches, {value: "EOF", start: offset, end: offset, eof: true}]

    matches.push({start: symbol.index! + offset, end: symbol.index! + offset + symbol[0].length, value: symbol[0], eof: false})
